...
mtab_migrate ()
{
	...
	if [ -L "/etc/mtab" ] && [ "$(readlink "/etc/mtab")" = "/proc/mounts" ]; then
		...
		log_success_msg "Creating compatibility symlink from /etc/mtab to /proc/mounts."
...
fstab_migrate ()
{
	if [ ! -r "/etc/fstab.d/live" ] || [ -s "/etc/fstab" ]; then
		return 1
	fi

	if [ -L "/etc/fstab" ] && [ "$(readlink "/etc/fstab")" = "/etc/fstab.d/live" ]; then
		:
	else
		log_success_msg "Creating live mode fstab file"
		(cp -f /lib/live/mount/medium/install/backend.parm /var/cache/horus/backend.parm) 2> /dev/null
		(cp -f /lib/live/mount/medium/install/horus.conf /var/cache/horus/horus.conf) 2> /dev/null
		rm -f "/etc/fstab" || return 1
		ln -fs "/etc/fstab.d/live" "/etc/fstab" || return 1
	fi
