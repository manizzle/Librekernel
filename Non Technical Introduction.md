...
- Different services that tract of all the data transaction like Internet proxy, clamav, surikata proxy, snort unbound DNS lists, iptables etc
...
- Doesn't include any non-free firmware (bugs aside), but it allows users to load non-free firmware if they wish to do so, which might pose the same kind of threats like, Trojans, tracking, kernel training with malicious software, sniffing etc.
